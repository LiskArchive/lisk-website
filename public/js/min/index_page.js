var videobackground, videoState = '';
// Video Init
/*$(document).ready(function(){
	if(!(html_var.hasClass('mobile') || html_var.hasClass('tablet'))){
		videobackground = new $.backgroundVideo($('.bg_video_wrapper'), {
			"align"   : "centerXY",
			"width"   : 1280,
			"height"  : 720,
			"path"    : "video/",
			"filename": "shutterstock_v1833242_1",
			"types"   : ["mp4", "webm"],
			"autoplay": false,
			"autobuffer" : true,
			"poster"  : '/i/0sec-hd.png',
			"preload" : "auto",
			"loop"    : false
		});

		videobackground.$videoEl.bind("ended", function(){
			videoState = 'stop';
			//videobackground.$videoEl.hide();
		});

		videobackground.videoEl.oncanplay = function(){
			videoState = 'play';
			videobackground.videoEl.play();

		//	setTimeout(function(){
		//		html_var.addClass('last_frame');
		//	}, 1000);

		}
	}

	browserWindow.on('scroll', function(e){
		if(doc_var.scrollTop() > browserWindow.height()){
			if(videoState == 'play'){
				videoState = 'pause';
...
var tokenRegex = /\{([^\}]+)\}/g,
    ...
    Raphael.fn.popup = function (X, Y, set, pos, ret) {
        ...
        var r = 1,
            bb = set.getBBox(),
            w = Math.round(bb.width)+48,
            h = Math.round(bb.height)+28,
            x = Math.round(bb.x) - r,
            y = Math.round(bb.y) - r,
